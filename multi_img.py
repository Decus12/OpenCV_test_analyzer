...
"""***************************************************************
********Excel writing preparion and using multiple files**********
***************************************************************"""
if caller == 0:
    workbook = xlsxwriter.Workbook('tests.xlsx')
else:
    workbook = xlsxwriter.Workbook('Classified tests.xlsx')
    
worksheet = workbook.add_worksheet('Test results')
worksheet.write('A1', 'File')
worksheet.write('B1', 'Reference Result')
worksheet.write('C1', 'C-line Intensity')
worksheet.write('D1', 'T-line Intensity')
worksheet.write('E1', 'T-line size')
worksheet.write('F1', 'C-line place')
worksheet.write('G1', 'T-line place')
worksheet.write('H1', 'Ratio')
    ...
    for f in filenames:
        try:
            ...
            ***************************************************************"""
            ...
            #Here we crop the central part of the input image in order to find the test area from it with template matching. We also crop the colored version for later use with intensities.
            ...
            gray_middle = gray[imgMidpoint[0]-int(imageSize[0]/5):imgMidpoint[0]+int(imageSize[0]/5), imgMidpoint[1]-int(imageSize[1]/4):imgMidpoint[1]+int(imageSize[1]/4)]

            image_middle = image[imgMidpoint[0]-int(imageSize[0]/5):imgMidpoint[0]+int(imageSize[0]/5), imgMidpoint[1]-int(imageSize[1]/4):imgMidpoint[1]+int(imageSize[1]/4)]
            ...
            ***************************************************************"""
            ...
            Cmid = Cline[ClineMid[0]-5:ClineMid[0]+5, ClineMid[1]-3:ClineMid[1]+3]
            #Cline = cv2.cvtColor(Cmid, cv2.COLOR_BGR2GRAY) 
            ...
            ***************************************************************"""
            ...
            #just slightly to get a threshold value for detecting the T-line.
            ...
            #the image and we need to have some tolerance for it. Same kind of scaling is udes to the right side of the T-line with scale from 25% to 35% left off.
            ...
            res_name = str(f)
            cv2.imwrite("res_results/" + res_name,cropedcolor)    
            #print avg_C
            #print avg_T
            #print avg_bg
            ...
            ***************************************************************"""
            ...
            n = n+1
            worksheet.write(n, 0, f)
            worksheet.write(n, 1, ref)
            worksheet.write(n, 2, avg_C)
            worksheet.write(n, 3, avg_T)
            worksheet.write(n, 4, size)
            worksheet.write(n, 5, bottom_rightC[0])
            worksheet.write(n, 6, locT)
            worksheet.write(n, 7, ratio)
